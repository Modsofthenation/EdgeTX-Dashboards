...
const BASE_STEPS: InstallStep[] = [
  {
    ...
    verify: "On the radio, browse SD card and confirm main.lua exists under WIDGETS/<name>/.",
  },
  {
    id: "discover",
    ...
    verify: "Telemetry page lists sensors like RxBt, RQLY, Alt (names vary by protocol). Values update live.",
  },
  {
    id: "add-widget",
...
const PROTOCOL_NOTES: Record<TelemetryProtocol, { label: string; prerequisites: string[]; extra?: InstallStep[] }> = {
  betaflight: {
    ...
    extra: [
      {
        ...
        detail: "With GPS enabled, expect ~17 CRSF sensors after Discover new. Restart the radio once if all values show zero after first install.",
        verify: "RxBt, Curr, RQLY, and FM sensors show non-zero values when armed or connected.",
      },
    ],
  },
  rotorflight: {
    ...
    extra: [
      {
        ...
        verify: "After power-on, custom sensors (RPM, HSpd, etc.) appear after Discover new.",
      },
      {
        id: "rf-rediscover",
        ...
        verify: "Rotorflight-specific sensors match those referenced in the widget source.",
      },
    ],
  },
  "generic-crsf": {
    ...
    extra: [
      {
        ...
        verify: "Each sensor referenced in the widget appears on the Telemetry page.",
      },
    ],
  },
};
...
const VERIFICATION: InstallStep[] = [
  {
    ...
    detail: "Open full-screen mode. No text or gauges should be clipped at edges.",
    verify: "All labels readable on 480×320 display.",
  },
  {
    id: "v-telem",
    title: "Telemetry values update",
    detail: "Move sticks, change altitude, or throttle — displayed values should change (not stay at zero).",
    verify: "At least battery and link sensors update in real time.",
  },
  {
    id: "v-restart",
    title: "Survives radio restart",
    detail: "Power cycle the radio. Widget should still appear and telemetry should resume after link reconnects.",
    verify: "Widget loads automatically on the configured main view.",
  },
];

const TROUBLESHOOTING = [
  { issue: "Widget not in picker", fix: "Folder name must match widget name (≤10 chars). Reboot radio after copying files." },
  { issue: "All values zero", fix: "Run Discover new with FC powered. Restart radio. Check receiver bind and telemetry wiring." },
  { issue: "Layout clipped", fix: "Use full-screen mode. Widget targets 480×320 (TX15)." },
  { issue: "Missing Rotorflight sensors", fix: "Enable rf2bg special function, delete sensors, rediscover with FC on." },
  { issue: "Preview differs from radio", fix: "Web preview uses mock telemetry; actual colors/layout may vary slightly on hardware." },
];

export function buildInstallGuide(protocol: TelemetryProtocol, widgetName?: string): InstallGuide {
