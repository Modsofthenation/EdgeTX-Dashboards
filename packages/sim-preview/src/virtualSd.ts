...
export const PLACEHOLDER_MODEL_PNG = new Uint8Array([
  ...
  0x48, 0x44, 0x52, 0x00, 0x00, 0x00, 0x38, 0x00, 0x00, 0x00, 0x28, 0x08, 0x02,
  0x00, 0x00, 0x00, 0x24, 0x32, 0xae, 0xd2, 0x00, 0x00, 0x00, 0x45, 0x49, 0x44,
  0x41, 0x54, 0x78, 0x9c, 0xed, 0xce, 0x41, 0x09, 0x00, 0x30, 0x0c, 0x00, 0xb1,
  0xaa, 0x9d, 0x7f, 0x09, 0xb3, 0xd0, 0x47, 0x60, 0x0c, 0x2e, 0x0a, 0x32, 0xe7,
  0x13, 0xf3, 0x3a, 0xb0, 0x55, 0x54, 0x2b, 0xaa, 0x15, 0xd5, 0x8a, 0x6a, 0x45,
  0xb5, 0xa2, 0x5a, 0x51, 0xad, 0xa8, 0x56, 0x54, 0x2b, 0xaa, 0x15, 0xd5, 0x8a,
  0x6a, 0x45, 0xb5, 0xa2, 0x5a, 0x51, 0xad, 0xa8, 0x56, 0x54, 0x2b, 0xaa, 0x15,
  0xd5, 0x2e, 0x90, 0x39, 0x76, 0x17, 0x51, 0xa5, 0xfc, 0x68, 0x00, 0x00, 0x00,
...
export function planWidgetDeploy(
  source: string,
  fallbackFolderName?: string,
): WidgetDeployPlan {
  const parsedName = extractWidgetName(source);
  const folderName = sanitizeWidgetFolderName(
    parsedName ?? fallbackFolderName ?? "Widget",
  );
  const widgetName = parsedName
    ? sanitizeWidgetFolderName(parsedName)
    : folderName;
  return {
    folderName,
    widgetName,
