...
export const ROTORFLIGHT_HELI_PREFABS: PrefabSection[] = [
  ...
  },
  {
    ...
    telemetryNotes: [
      ...
      "Flight count reads Global Variable GV1 (same as Insert → Companion suites → Flights counter). Run flt_count tool or arm logger to bump it.",
    ],
    createSrcBindings: {},
    ...
    refreshLines: [
      ...
      "local flightN = model.getGlobalVariable(1, 0) or 0",
      'lcd.drawText(90, 152, tostring(flightN) .. " Flights", SMLSIZE + CENTER + WHITE)',
    ],
  },
  {
    id: "rf-governor-card",
  ...
  },
  {
    ...
    refreshLines: [
      "local motorOn = (model.getGlobalVariable(2, 0) or 1) ~= 0",
      ...
      'local ampsStr = (motorOn and amps > 0) and tostring(math.ceil(amps)) or "--"',
      "local cellV = vcel > 0 and vcel or (vpack > 0 and vpack / 6 or 0)",
      'local cellStr = (motorOn and cellV > 0) and string.format("%.2f", cellV) or "--"',
      'local becStr = (motorOn and vbec > 0) and string.format("%.1f", vbec) or "--"',
      'local escStr = (motorOn and esct > 0) and tostring(math.floor(esct + 0.5)) or "--"',
