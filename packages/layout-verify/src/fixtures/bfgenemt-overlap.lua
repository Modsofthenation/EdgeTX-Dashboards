...
-- BfGenemt — Tiny Whoop quad overview (dark yellow/cyan, altitude hero)
...
local function stripBlockH(showAtt, showCapa)
  ...
  if showAtt and showCapa then
    return cardPad + stripInnerRowH() * 2 + LH.GAP + cardPad
  end
  return cardPad + stripInnerRowH() + cardPad
end
...
local function refresh(widget, event, touchState)
  ...
  local hdg = telem(widget.src.hdg)
  ...
  local maxAltM = widget.maxAltM or 88
  local altPct = 0
  if alt ~= 0 then
    altPct = math.max(0, math.min(100, alt / maxAltM * 100))
  end
  local altFillPct = math.floor(altPct + 0.5)
  ...
  local hdgStr = "--"
  if hdg ~= 0 then
    hdgStr = tostring(math.floor(hdg + 0.5))
  end
  ...
  local yAlt = gaugeCy - math.floor(gaugeBlockH / 2)
  local yAltUnit = yAlt + LH.DBL + LH.GAP
  local ySatLbl = gaugeCy + rOut + 6
  local ySatVal = ySatLbl + LH.SML + LH.GAP
  local ySatUnit = ySatVal + LH.MID + LH.GAP
  local rGaugeInner = textRowRect(gaugeCx - 48, yAlt, 96, gaugeBlockH)
  local rGaugeSatLeft = textRowRect(gaugeCx - 80, ySatVal, 72, LH.MID + LH.GAP + LH.SML)
  local rGaugeSatRight = textRowRect(gaugeCx + 8, ySatVal, 72, LH.MID + LH.GAP + LH.SML)
  ...
  if anyTextForeignOverlap(textFootprintRects, layoutRects) and rOut > 20 then
    ...
    rGauge = rect(gaugeCx - rOut, gaugeTop, rOut * 2, gaugeSatBottom - gaugeTop)
    layoutRects[#layoutRects] = rGauge
    yAlt = gaugeCy - math.floor(gaugeBlockH / 2)
    yAltUnit = yAlt + LH.DBL + LH.GAP
    ySatLbl = gaugeCy + rOut + 6
    ySatVal = ySatLbl + LH.SML + LH.GAP
    ySatUnit = ySatVal + LH.MID + LH.GAP
    rGaugeInner = textRowRect(gaugeCx - 48, yAlt, 96, gaugeBlockH)
    rGaugeSatLeft = textRowRect(gaugeCx - 80, ySatVal, 72, LH.MID + LH.GAP + LH.SML)
    rGaugeSatRight = textRowRect(gaugeCx + 8, ySatVal, 72, LH.MID + LH.GAP + LH.SML)
    textFootprintRects = {}
    if showLink then
      textFootprintRects[#textFootprintRects + 1] = { rect = rBarsPctLeft, owner = rBars }
      textFootprintRects[#textFootprintRects + 1] = { rect = rBarsPctRight, owner = rBars }
  ...
  local valA = startA + span * (altPct / 100)
  ...
  local altFillColor = CYAN
  if altFillPct >= 75 then
    altFillColor = YELLOW
  elseif altFillPct >= 50 then
    altFillColor = GREEN
  end
  ...
  lcd.drawFilledRectangle(rHeader.x, rHeader.y, rHeader.w, rHeader.h, DARKGREY)
  lcd.drawLine(0, headerH - 1, w, headerH - 1, SOLID, CYAN)
  lcd.drawFilledRectangle(pad, 4, w - pad * 2, 4, YELLOW)

  lcd.drawFilledRectangle(rArm.x, rArm.y, rArm.w, rArm.h, bannerFill)
  if not armed then
    lcd.drawLine(0, rectBottom(rArm) - 1, w, rectBottom(rArm) - 1, SOLID, CYAN)
  end
  ...
  if showLink then
    ...
    if linkFillW > 0 then
      lcd.drawFilledRectangle(leftBarX + barInset, trackY + barInset, linkFillW, barH - barInset * 2, GREEN)
    end

    local battFillW = math.floor((barW - barInset * 2) * battFillPct / 100)
    local battBarColor = YELLOW
    ...
    if battFillW > 0 then
      lcd.drawFilledRectangle(rightBarX + barInset, trackY + barInset, battFillW, barH - barInset * 2, battBarColor)
    end
  end
  ...
  if valA > 360 then
    lcd.drawAnnulus(gaugeCx, gaugeCy, rIn, rOut, startA, 360, altFillColor)
    lcd.drawAnnulus(gaugeCx, gaugeCy, rIn, rOut, 0, valA - 360, altFillColor)
  elseif altPct > 0 then
    lcd.drawAnnulus(gaugeCx, gaugeCy, rIn, rOut, startA, valA, altFillColor)
  end
  ...
  lcd.drawFilledCircle(gaugeCx, gaugeCy, discR, DARKGREY)

  if stripH > 0 then
    if showAtt and showCapa then
      lcd.drawFilledRectangle(rStripLeft.x, rStripLeft.y, rStripLeft.w, rStripLeft.h, DARKGREY)
      lcd.drawRectangle(rStripLeft.x, rStripLeft.y, rStripLeft.w, rStripLeft.h, CYAN)
      lcd.drawFilledRectangle(rStripRight.x, rStripRight.y, rStripRight.w, rStripRight.h, DARKGREY)
      lcd.drawRectangle(rStripRight.x, rStripRight.y, rStripRight.w, rStripRight.h, CYAN)
    else
      lcd.drawFilledRectangle(rStripFull.x, rStripFull.y, rStripFull.w, rStripFull.h, DARKGREY)
      lcd.drawRectangle(rStripFull.x, rStripFull.y, rStripFull.w, rStripFull.h, CYAN)
    end
  end

  if showGPS then
    lcd.drawFilledRectangle(rGps.x, rGps.y, rGps.w, rGps.h, DARKGREY)
    lcd.drawRectangle(rGps.x, rGps.y, rGps.w, rGps.h, CYAN)
  end

  lcd.drawFilledRectangle(rFooter.x, rFooter.y, rFooter.w, rFooter.h, DARKGREY)
  lcd.drawLine(0, footerY, w, footerY, SOLID, CYAN)
  lcd.drawFilledRectangle(chipX, footerY + 4, chipW, footerH - 8, armed and ORANGE or YELLOW)

  -- === phase 2: text ===
  lcd.drawText(pad, 12, "BfGenemt", MIDSIZE + YELLOW)
  lcd.drawText(w - pad, 12, "TINY WHOOP", SMLSIZE + RIGHT + CYAN)

  lcd.drawText(w / 2, rArm.y + 6, statusLabel, MIDSIZE + CENTER + statusColor)

  if showTimer then
    lcd.drawText(pad, rTimer.y, timerLabel, SMLSIZE + GREY)
    lcd.drawText(w / 2, rTimer.y, timerStr, DBLSIZE + CENTER + YELLOW)
  end

  if showLink then
    lcd.drawText(leftBarX, barsY, "LINK", SMLSIZE + GREY)
    lcd.drawText(rightBarX, barsY, "BATTERY", SMLSIZE + GREY)
    lcd.drawText(leftBarX + barW - 4, barsPctY, linkFillPct .. "%", SMLSIZE + RIGHT + GREEN)
    lcd.drawText(rightBarX + barW - 4, barsPctY, batPctStr, SMLSIZE + RIGHT + YELLOW)
  end

  lcd.drawText(gaugeCx, yAlt, altStr, DBLSIZE + CENTER + YELLOW)
  lcd.drawText(gaugeCx, yAltUnit, "ALT m", SMLSIZE + CENTER + GREY)
  lcd.drawText(gaugeCx - 56, ySatVal, voltsStr, MIDSIZE + RIGHT + CYAN)
  lcd.drawText(gaugeCx - 56, ySatUnit, "V", SMLSIZE + RIGHT + GREY)
  lcd.drawText(gaugeCx + 56, ySatVal, ampsStr, MIDSIZE + LEFT + GREEN)
  lcd.drawText(gaugeCx + 56, ySatUnit, "A", SMLSIZE + LEFT + GREY)

  if stripH > 0 then
    local sy = stripY + 8
    if showAtt and showCapa then
      lcd.drawText(rStripLeft.x + 10, sy, "PITCH", SMLSIZE + GREY)
      lcd.drawText(rStripLeft.x + 10, sy + LH.SML + LH.GAP, ptchStr, MIDSIZE + CYAN)
      lcd.drawText(rStripLeft.x + 10, sy + stripInnerRowH() + LH.GAP, "ROLL", SMLSIZE + GREY)
      lcd.drawText(rStripLeft.x + 10, sy + stripInnerRowH() + LH.GAP + LH.SML + LH.GAP, rollStr, MIDSIZE + CYAN)
      lcd.drawText(rStripRight.x + 10, sy, "USED", SMLSIZE + GREY)
      lcd.drawText(rStripRight.x + 10, sy + LH.SML + LH.GAP, capaStr, MIDSIZE + YELLOW)
      lcd.drawText(rStripRight.x + 10, sy + LH.SML + LH.GAP + LH.MID + LH.GAP, "mAh", SMLSIZE + GREY)
    elseif showAtt then
      local halfW = math.floor(rStripFull.w / 2)
      lcd.drawText(rStripFull.x + 10, sy, "PITCH", SMLSIZE + GREY)
      lcd.drawText(rStripFull.x + 10, sy + LH.SML + LH.GAP, ptchStr, MIDSIZE + CYAN)
      lcd.drawText(rStripFull.x + halfW + 10, sy, "ROLL", SMLSIZE + GREY)
      lcd.drawText(rStripFull.x + halfW + 10, sy + LH.SML + LH.GAP, rollStr, MIDSIZE + CYAN)
    elseif showCapa then
      lcd.drawText(rStripFull.x + 10, sy, "USED", SMLSIZE + GREY)
      lcd.drawText(rStripFull.x + 10, sy + LH.SML + LH.GAP, capaStr, MIDSIZE + YELLOW)
      lcd.drawText(rStripFull.x + 10, sy + LH.SML + LH.GAP + LH.MID + LH.GAP, "mAh", SMLSIZE + GREY)
    end
  end

  if showGPS then
    local gy = gpsY + 8
    lcd.drawText(pad + 10, gy, "SPD", SMLSIZE + GREY)
    lcd.drawText(pad + 10, gy + LH.SML + LH.GAP, gspdStr, MIDSIZE + CYAN)
    lcd.drawText(pad + 120, gy, "HDG", SMLSIZE + GREY)
    lcd.drawText(pad + 120, gy + LH.SML + LH.GAP, hdgStr, MIDSIZE + CYAN)
    lcd.drawText(w - pad - 10, gy, "SATS", SMLSIZE + RIGHT + GREY)
    lcd.drawText(w - pad - 10, gy + LH.SML + LH.GAP, satsStr, MIDSIZE + RIGHT + GREEN)
  end

  lcd.drawText(chipX + math.floor(chipW / 2), footerY + 8, chipLabel, SMLSIZE + CENTER + BLACK)
  lcd.drawText(trssX, footerY + 8, trssText, SMLSIZE + CYAN)
  lcd.drawText(fmRight, footerY + 8, fmDisplay, SMLSIZE + RIGHT + YELLOW)
end
