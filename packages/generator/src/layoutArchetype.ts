...
const ARCHETYPES: Record<LayoutArchetypeId, LayoutArchetypeHint> = {
  "card-grid": {
    ...
    summary:
      "Two-column metric cards with a full-width lower band and status footer.",
    layoutNotes:
  ...
  "hero-minimal": {
    ...
    title: "Hero minimal",
    summary:
      "One DBLSIZE hero metric centered or top-right with 2–3 small secondary readouts.",
    layoutNotes:
  ...
  "strip-board": {
    ...
    summary:
      "Three or four equal vertical strips across the width, each with label + value.",
    layoutNotes:
  ...
  "flight-logger-suite": {
    ...
    summary:
      "Main dashboard plus SCRIPTS/TELEMETRY logger and optional log viewer tool.",
    layoutNotes:
  ...
  "battery-tool-suite": {
    ...
    summary:
      "Battery-focused dashboard plus TOOLS script to pick active pack / cell count.",
    layoutNotes:
...
const CARD_GRID_ARCHETYPES = new Set<LayoutArchetypeId>([
  "card-grid",
  "heli-rotorflight",
]);
...
export function suggestLayoutArchetype(
  ...
  if (
    /logger|log viewer|flight log|blackbox|gpx|history|record flight/.test(p)
  ) {
    return ARCHETYPES["flight-logger-suite"];
  }
  if (
    /battery select|pack select|cell count|mah preset|battery preset/.test(p)
  ) {
  ...
  const heliKeywords =
    /heli|rotorflight|headspeed|goblin|logo|tail|dbk|hspd/.test(p);
  ...
  if (protocol === "rotorflight" && (heliKeywords || explicitHeliBoard)) {
    if (
      /vibrant|colorful|colourful|neon|bright|bold color|saturated|lively/.test(
        p,
      )
    ) {
      const vibrantHeli: LayoutArchetypeId[] = [
        "strip-board",
        "hero-minimal",
        "telemetry-dense",
      ];
...
export function shouldIncludeCardStarter(
