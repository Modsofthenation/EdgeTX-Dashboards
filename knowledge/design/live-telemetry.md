...
| Preview **enrich** (default on for Rotorflight)     | Synthetic fill for missing HSpd/Gov/Vbec/… so heli boards stay readable |
...
3. Install the **RF sensor dump** companion (Layout Insert → RF heli electric pack, or sensor-dump suite) and run it under SYS → Tools to confirm `HSpd`/`Gov`/`Vbec`
...
- `apps/web/src/lib/companionSuites.ts` (`sensor-dump`, `rf-heli-electric`)
- `knowledge/design/rotorflight-heli-sections.md`
